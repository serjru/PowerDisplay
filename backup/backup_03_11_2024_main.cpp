    ...
    for (int row = 0; row < ICON_SIZE; row++) {
        for (int byte_col = 0; byte_col < (ICON_SIZE / 8); byte_col++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                if (byte & (0x80 >> bit)) {  // Check if bit is set, starting from MSB
                    int pixel_x = x + (byte_col * 8) + bit;  // Calculate actual x coordinate
                    int pixel_y = y + row;
                    epd_draw_pixel(pixel_x, pixel_y, 0x00, framebuffer);
                }
            }
        }
    }
